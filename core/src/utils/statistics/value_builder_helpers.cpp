...
std::string MakeConflictMessage(const formats::json::ValueBuilder& original, const formats::json::ValueBuilder& patch) {
...
void CheckedMerge(formats::json::ValueBuilder& original, formats::json::ValueBuilder&& patch) {
    if (patch.IsObject() && original.IsObject()) {
        ...
        UASSERT_MSG(
            original.IsNull() ||  // TODO: remove IsNull()
...
bool AreAllMetricsNumbers(const formats::json::Value& json) {
    ...
    UASSERT_MSG(
        !path.has_value(),
        "Some metrics are not numbers, path: " + *path +
            ". Value: " + ToString(formats::common::GetAtPath(json, formats::common::SplitPathString(*path)))
    );
    return true;
}
